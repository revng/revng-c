...
[[maybe_unused]] static bool verifyNames();
[[maybe_unused]] static bool verify(const model::Type &ModelType, bool Assert);
[[maybe_unused]] static bool verify(const model::QualifiedType &ModelType,
                                    bool Assert);
[[maybe_unused]] static bool verify(const model::Binary &Tree, bool Assert);
...
BOOST_AUTO_TEST_CASE(UnionTypes) {
...

  {
    ...
    revng_check(not verify(*Union, false) == verifyNames());
    revng_check(not verify(*T, false));
