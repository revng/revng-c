...
mlir::ParseResult FunctionOp::parse(OpAsmParser &Parser,
  ...
  if (Parser.parseSymbolName(SymbolNameAttr,
...
void FunctionOp::print(OpAsmPrinter &Printer) {
  ...
  function_interface_impl::printFunctionSignature(Printer,
                                                  *this,
                                                  FunctionTypeAttr.getArgumentTypes(),
                                                  /*isVariadic=*/false,
                                                  FunctionTypeAttr.getResultTypes());
...
mlir::ParseResult CallOp::parse(OpAsmParser &Parser, OperationState &Result) {
  ...
  if (Parser.resolveOperand(FunctionOperand,
                            FunctionType,
                            Result.operands).failed())
    return mlir::failure();

  if (Parser.resolveOperands(ArgumentOperands,
                             ArgumentTypes,
                             ArgumentOperandsLoc,
                             Result.operands).failed())
    return mlir::failure();
...
void CallOp::print(OpAsmPrinter &Printer) {
  ...
  for (auto [ArgumentT, ParameterT] : llvm::zip_equal(ArgumentTypes,
                                                      ParameterTypes)) {
